...
export default function Orders() {
  ...
  const { data: orders, isLoading } = useQuery({
    ...
    queryFn: () => api.getOrders({ status: statusFilter }).then(res => res.json()),
  });
                  ...
                  {orders.map((orderData: any) => {
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-muted/50" data-testid={`row-order-${order.id}`}>
                        ...
                        <td className="p-4 text-sm text-foreground" data-testid={`text-item-count-${order.id}`}>
                          {orderData.itemCount} items
                        </td>
                        <td className="p-4 font-medium text-foreground" data-testid={`text-order-total-${order.id}`}>
                        ...
                        <td className="p-4 text-sm text-muted-foreground" data-testid={`text-order-date-${order.id}`}>
