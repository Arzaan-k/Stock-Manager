...
interface ConversationState {
  ...
  userName?: string;
  userPhone: string;
  currentFlow: 'idle' | 'awaiting_name' | 'adding_stock' | 'creating_order' | 'collecting_order_details';
...
export class EnhancedWhatsAppService {
  ...
  private async detectIntent(message: string, state: ConversationState): Promise<{
    ...
    // Check for order creation keywords - prioritize "order" keyword
  ...
  private async handleStockAddition(userPhone: string, message: string, state: ConversationState): Promise<string> {
    ...
    if (state.pendingStockAddition?.awaitingConfirmation) {
      if (/^(yes|y|confirm|ok|correct)$/i.test(message.trim())) {
        ...
        try {
          ...
          // Create stock movement record with user attribution
          await storage.createStockMovement({
            ...
            // userId is commented out as it requires a valid user UUID from the users table
            // userId: state.userName || userPhone,
          });
          ...
          await storage.createWhatsappLog({
            ...
            quantity,
            aiResponse: `Failed to add ${quantity} units: ${error.message}`,
            status: "failed",
            meta: {
              error: error.message,
              addedBy: state.userName,
    ...
    if (!state.userName) {
      ...
      return `📦 Found product: ${product.name} (SKU: ${product.sku})\n` +
      ...
      case 'confirming_order': {
        if (/^(confirm|yes|y|ok|place order)$/i.test(message.trim())) {
          ...
          try {
            ...
            let customer = (await storage.getCustomers()).find(
              c => c.phone === order.customerPhone || c.name === order.customerName
            );
    ...
    try {
      ...
      try {
        ...
        await storage.createWhatsappLog({
          ...
          imageUrl,
          confidence: result.matches.length > 0 ? result.matches[0].confidence : 0,
        ...
        if (result.matches.length === 0) {
          ...
          if (result.extractedText && result.extractedText.length > 2) {
            response += `I detected this text: "${result.extractedText}"\n\n`;
          }
        ...
        if (result.matches.length === 1 && result.matches[0].confidence > 0.8) {
          ...
          if (result.extractedText && result.extractedText.length > 2) {
            response += `💬 Detected text: "${result.extractedText}"\n\n`;
          }
  ...
  private async handleProductSelection(userPhone: string, selectionText: string, state: ConversationState): Promise<string> {
    ...
    return ""; // Response already sent by handleProductIdentified
  }
    ...
    try {
      ...
      if (message.type === "text") {
        ...
        if (imageId) {
          ...
          await this.sendWhatsAppMessage(userPhone,
            "❌ No image data received. Please try sending the image again.");
        }
      }
